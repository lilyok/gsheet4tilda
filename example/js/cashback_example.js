$().ready(function(){
    var email = 'lil.lilyok@gmail.com';
    getData(
        {
            url : "https://docs.google.com/spreadsheets/d/1EL6Kr649Rf1T-qtJYWkihLZ6gPZZMKYlF5y9_W9UKFI/edit?usp=sharing",
            req : "select A, H, I where B='" + email + "'",  // ID, parentID, PrevSum, CurSum, AdditionalText
            range: 'A1:I',
            sheet: 'cashback1',
            labels: ['rowId', 'spended', 'cashback']
        },
    );
})

function useCashback() {
    if ($("#cashback_value").prop("nodeName") == 'INPUT') {
        var availableCashback = parseFloat($("#cashback_value").val());
    } else {
        var availableCashback = parseFloat($("#cashback_value").text());
    }
    var rowId = parseInt($("#rowId_value").text());
    var spendedCashback = parseFloat($("#spended").text());

    if (isNaN(rowId) || isNaN(availableCashback) || isNaN(spendedCashback)) {
        return;
    }

    if (availableCashback == 0) {
        return;
    }

    //TODO calculate max max(70% from purchase, availableCashback)
    let cashBack = availableCashback;
    availableCashback = availableCashback - cashBack;


    var params = {
        "url": "https://docs.google.com/spreadsheets/d/1EL6Kr649Rf1T-qtJYWkihLZ6gPZZMKYlF5y9_W9UKFI/edit?usp=sharing",
        "robot": "sansmots@sansmots.iam.gserviceaccount.com",
        "id": "",
        "sheet": "count_cashback!",
        "range": `K${rowId}:K${rowId}`,
        "values": [[cashBack + spendedCashback]],
        "htmlField2update": {"cashback_value": availableCashback, "spended_value": cashBack + spendedCashback},
    }
    setData(params);
}